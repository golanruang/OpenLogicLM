...
class FOL_Parser:
    ...
    def parse_text_FOL_to_tree(self, rule_str):
        ...
        """
        ## TODO: there is a bug for the reorder function, cannot handle ¬∀x (P(x) -> Q(x)) correctly
        ## TODO: how to convert ¬∀x (P(x) -> Q(x)) into ∃x (P(x) -> ¬Q(x))
    ...
    def symbol_resolution(self, tree):
        ...
        self.preorder_resolution(main_tree, lvars, consts, preds)
...
if __name__ == '__main__':
    ...
    # str_fol = '∀x (Athlete(x) ∧ WinsGold(x, olympics) → OlympicChampion(x))' 
    # str_fol = '∀x (Penguin(x) → Bird(x))'
    # prompt GPT to also add the predicates
    # better to build parser support for multiple parameter functions
    ...
    isFOL, lvars, consts, preds = parser.symbol_resolution(tree)
    print('isFOL: ', isFOL)
    print('lvars: ', lvars)
    print('consts: ', consts)
    print('preds: ', preds)